...
async function getAccessContext(params: Params) {
  ...
  return { backendBase, token, id };
}

export async function GET(_request: Request, { params }: Params) {
  const context = await getAccessContext({ params });
  if ("error" in context) {
    return context.error;
  }

  try {
    const response = await fetch(`${context.backendBase}/api/v1/citizen/issues/${context.id}`, {
      method: "GET",
