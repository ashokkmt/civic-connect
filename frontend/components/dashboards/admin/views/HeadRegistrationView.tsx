...
import type { HeadRow } from "@/components/dashboards/admin/types";
    ...
    <section className="space-y-6">
      ...
      <Card>
        <CardBody className="space-y-4">
          <h2 className="text-lg font-semibold text-zinc-900 dark:text-zinc-100">Registered Department Heads</h2>
          <Table headers={["Name", "Email", "Department Assignment"]}>
            {headRows.map((head) => (
              <tr key={head.id} className="border-b border-[var(--border)] last:border-0">
                ...
                <td className="px-4 py-3 text-sm text-zinc-700 dark:text-zinc-200">{head.departmentId}</td>
              </tr>
            ))}
            {headRows.length === 0 ? (
              <tr>
