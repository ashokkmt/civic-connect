...
import { Building2, CheckCircle2, ClipboardList, ShieldUser, Siren, TrendingUp } from "lucide-react";
    ...
    <section className="space-y-6">
      <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-5">
        <MetricCard
          ...
          note="Across all municipalities"
        />
        <MetricCard
          label="Department Heads"
          value={overviewStats.totalHeads}
          icon={<ShieldUser className="h-5 w-5 text-indigo-600" />}
          note="Leadership coverage"
        />
        <MetricCard
          label="Total Issues Reported"
          value={overviewStats.totalIssuesReported}
          icon={<ClipboardList className="h-5 w-5 text-blue-600" />}
          note="From admin-visible metrics"
        />
        <MetricCard
          label="Total Issues Resolved"
          value={overviewStats.totalIssuesResolved}
          icon={<CheckCircle2 className="h-5 w-5 text-emerald-600" />}
          note="Cross-department completion"
        />
        <MetricCard
          label="Pending Escalations"
          value={overviewStats.pendingEscalations}
      ...
      <div className="space-y-3">
        <div className="flex flex-wrap items-center justify-between gap-2">
          <div>
            <h2 className="text-lg font-semibold text-slate-900 dark:text-slate-100">Department Performance Comparison</h2>
            <p className="text-sm text-slate-500 dark:text-slate-400">Review issue volume and resolution outcomes by department.</p>
          </div>
          <Badge tone="accent">System Metrics</Badge>
        </div>

        <Table variant="slate" headers={["Department", "Head", "Total Issues", "Resolved Issues", "Success Rate"]}>
...
function MetricCard({ label, value, icon, note }: { label: string; value: number; icon: ReactNode; note: string }) {
  return (
    <Card className="border-slate-200 bg-white dark:border-slate-800 dark:bg-slate-900">
      <CardBody className="space-y-3">
        ...
        <div className="inline-flex items-center gap-1 text-xs font-medium text-emerald-600 dark:text-emerald-300">
